        ...
        <form onSubmit={placeOrder} className="lg:col-span-2 card p-4">
          ...
          <div className="mt-3 grid grid-cols-1 md:grid-cols-2 gap-3">
            <div>
              ...
              <input value={delivery.name} onChange={e=>setDelivery(d=>({ ...d, name: e.target.value }))} className="mt-1 w-full rounded-md border border-white/10 bg-[#111727] px-3 py-2 outline-none" />
            </div>
            <div>
              <label className="text-sm text-white/70">Email</label>
              <input value={delivery.email} onChange={e=>setDelivery(d=>({ ...d, email: e.target.value }))} type="email" className="mt-1 w-full rounded-md border border-white/10 bg-[#111727] px-3 py-2 outline-none" />
            </div>
            <div className="md:col-span-2">
              <label className="text-sm text-white/70">Address</label>
              <input value={delivery.address} onChange={e=>setDelivery(d=>({ ...d, address: e.target.value }))} className="mt-1 w-full rounded-md border border-white/10 bg-[#111727] px-3 py-2 outline-none" />
            </div>
            <div>
              <label className="text-sm text-white/70">City</label>
              <input value={delivery.city} onChange={e=>setDelivery(d=>({ ...d, city: e.target.value }))} className="mt-1 w-full rounded-md border border-white/10 bg-[#111727] px-3 py-2 outline-none" />
            </div>
            <div>
              <label className="text-sm text-white/70">Postal Code</label>
              <input value={delivery.postal} onChange={e=>setDelivery(d=>({ ...d, postal: e.target.value }))} className="mt-1 w-full rounded-md border border-white/10 bg-[#111727] px-3 py-2 outline-none" />
            </div>
          </div>
          ...
          {!billingSame && (
            <div className="mt-4">
              ...
              <div className="mt-3 grid grid-cols-1 md:grid-cols-2 gap-3">
                <div>
                  <label className="text-sm text-white/70">Full Name</label>
                  <input value={billing.name} onChange={e=>setBilling(b=>({ ...b, name: e.target.value }))} className="mt-1 w-full rounded-md border border-white/10 bg-[#111727] px-3 py-2 outline-none" />
                </div>
                <div>
                  <label className="text-sm text-white/70">Email</label>
                  <input value={billing.email} onChange={e=>setBilling(b=>({ ...b, email: e.target.value }))} type="email" className="mt-1 w-full rounded-md border border-white/10 bg-[#111727] px-3 py-2 outline-none" />
                </div>
                <div className="md:col-span-2">
                  <label className="text-sm text-white/70">Address</label>
                  <input value={billing.address} onChange={e=>setBilling(b=>({ ...b, address: e.target.value }))} className="mt-1 w-full rounded-md border border-white/10 bg-[#111727] px-3 py-2 outline-none" />
                </div>
                <div>
                  <label className="text-sm text-white/70">City</label>
                  <input value={billing.city} onChange={e=>setBilling(b=>({ ...b, city: e.target.value }))} className="mt-1 w-full rounded-md border border-white/10 bg-[#111727] px-3 py-2 outline-none" />
                </div>
                <div>
                  <label className="text-sm text-white/70">Postal Code</label>
                  <input value={billing.postal} onChange={e=>setBilling(b=>({ ...b, postal: e.target.value }))} className="mt-1 w-full rounded-md border border-white/10 bg-[#111727] px-3 py-2 outline-none" />
                </div>
              </div>
            </div>
          )}
            ...
            <div className="mt-3 grid gap-3">
              ...
              {paymentMethod === 'card' && (
                <div className="mt-2 grid grid-cols-1 md:grid-cols-3 gap-2">
                  <input placeholder="Card number" value={card.number} onChange={e=>setCard(c=>({ ...c, number: e.target.value }))} className="md:col-span-3 rounded-md border border-white/10 bg-[#111727] px-3 py-2" />
                  <input placeholder="MM/YY" value={card.expiry} onChange={e=>setCard(c=>({ ...c, expiry: e.target.value }))} className="rounded-md border border-white/10 bg-[#111727] px-3 py-2" />
                  <input placeholder="CVV" value={card.cvv} onChange={e=>setCard(c=>({ ...c, cvv: e.target.value }))} className="rounded-md border border-white/10 bg-[#111727] px-3 py-2" />
                </div>
              )}
            ...
            {items.map(it=> (
              <div key={it.id} className="py-3 flex items-center justify-between">
                <div>
                  ...
                  <div className="text-sm text-white/60">{it.qty} × ${it.price.toFixed(2)}</div>
                </div>
                <div className="font-semibold">${(it.qty*it.price).toFixed(2)}</div>
              </div>
            ))}
          </div>
          <div className="mt-4 flex items-center justify-between text-lg font-bold">Total: <span>${total.toFixed(2)}</span></div>
        </aside>
      </div>
    </div>
  )
