...
export default function ProductDetail() {
  ...
  // Directly add to cart without showing modal
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div className="p-3 flex items-center gap-3 overflow-auto">
              {images.map((src, i) => (
                <button
                  key={i}
                  onClick={() => setIndex(i)}
                  className={`flex-shrink-0 w-24 h-16 overflow-hidden rounded-md border ${i===index ? 'border-ice-400' : 'border-white/10'}`}
                >
        ...
        <div>
          ...
          <div className="mt-4 flex items-center gap-3">
            ...
            <button onClick={handleAddToCart} className={isIce ? 'btn-primary' : 'btn-secondary'}>
              Add to Cart
            </button>
          </div>
          ...
          <div className="mt-8">
            ...
            <ul className="mt-3 grid grid-cols-1 sm:grid-cols-2 gap-2 text-sm">
              {Object.entries(product.specs).map(([k, v]) => (
                <li key={k} className="card p-3 flex items-center justify-between">
